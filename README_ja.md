...
`--due-from` と `--due-to` は、`--done` と併用した場合も期限を絞り込みます。
完了日時の条件も指定した場合は、両方の条件を満たす項目だけを表示します。
期限の下限または上限を指定した場合、期限のない項目は表示しません。
例えば `rmd list --done --today --due-to "2026-06-30"` は、今日完了した項目のうち、
期限が2026年6月30日以前のものを表示します。
...
**同名リストについての注意:** `--list リスト名` と `RMD_DEFAULT_LIST` は、
アカウントを区別せず、名前だけでリストを選択します。同名のリストが複数ある場合は、
EventKit が返す最初の一致を使用します。そのため、`list --list リスト名` では
別の同名リストの項目が表示されず、`add` や `edit --list リスト名` では意図しない
リストに保存される可能性があります。名前で選択する場合は、この Mac でリマインダーを
有効にしているアカウント間でリスト名が重複しないように変更してください。
現在、アカウントやリスト ID を指定してリストを選択する機能はありません。
